...
async def handle_archive(filename: Path, target_folder: Path):
    ...
    # Беремо суфікс у файлу і прибираємо replace(filename.suffix, '')
    folder_for_file = target_folder / normalize(filename.name.replace(filename.suffix, ''))
    # створюємо папку для архіву з ім'ям файлу
...
async def handle_folder(folder: Path):
    ...
    try:
        ...
        print(f'не вдалося видалити папку {async_path}')
